...
class LongTrackTrainingDataset(Dataset):
    ...
    def __init__(
        ...
        self._precomputed_root_dir = precomputed_root_dir
        self._reid_on_gt_model_name = reid_on_gt_model_name
        ...
        self.global_ids = {
            seq_id_pair: global_id
            for global_id, seq_id_pair in enumerate(global_ids)
        }
    ...
    def prepare_data(self, frame_ix, seq):
        frame = seq[frame_ix]
        ...
        features_path = os.path.join(
            self._precomputed_root_dir,
            str(seq),
            self._reid_on_gt_model_name,
            "reid_on_gt",
            f"{frame_ix+1:06d}" + ".pth",
        )
        features = torch.load(features_path)
        ###############

        global_ids = torch.as_tensor(
            [self.global_ids[(str(seq), id_)] for id_ in ids.tolist()]
        )
    ...
    def merge_frame_data(self, past_frame_data, first_past_frame):
        ...
        return {
            ...
            "time": torch.as_tensor(
                [final_data[id_]["time"] for id_ in all_ids]
            ),
            "boxes": torch.as_tensor(
                torch.stack([final_data[id_]["boxes"] for id_ in all_ids])
            ),
            "features": torch.stack(
                [final_data[id_]["features"] for id_ in all_ids]
            ),
        }

    def __getitem__(self, idx):
        seq_ix, curr_frame_ix = self.seq_frames[idx]
        seq = self.sequences[seq_ix]
        first_past_frame = max(curr_frame_ix - self.max_past_frames - 1, 0)

        curr_frame_data = self.prepare_data(curr_frame_ix, seq)
        past_frame_datum = [
            self.prepare_data(frame_ix_, seq)
            for frame_ix_ in range(first_past_frame, curr_frame_ix)
        ]

        past_frame_data = self.merge_frame_data(
            past_frame_datum, first_past_frame
        )
        curr_frame_data["time"] = curr_frame_ix * torch.ones(
            len(curr_frame_data["vis"])
        )
