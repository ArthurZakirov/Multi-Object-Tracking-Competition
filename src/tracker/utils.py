...
def run_tracker(val_sequences, db, tracker, output_dir=None):
    ...
    mot_accums = []
    results_seq = {}
    for seq in tqdm(val_sequences, desc="sequences", leave=True):
        #break
        tracker.reset()
        now = time.time()
        ...
        if db is None:
          iterator = torch.utils.data.DataLoader(seq, batch_size=1, shuffle=False)
        else:
          iterator = db[str(seq)]
        with torch.no_grad():
            for frame in tqdm(iterator, desc="frame", leave=False):     
                tracker.step(frame)
