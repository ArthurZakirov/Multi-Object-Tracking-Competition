...
parser.add_argument(
    "--obj_detect_path", type=str, default="models/obj_detect/maskrcnn.pth"
)
    ...
    if args.tracker_or_detector_data == "detector":
        ...
        for sequence in tqdm(sequences, desc="detect sequence", leave=True):
            for frame_id, frame in tqdm(
                ...
                binary_masks = (
                    (detection["masks"][pedestrian] > 0.5)
                    .squeeze(1)
                    .int()
                    .cpu()
                )
        ...
        print("\nsave detections...")
        for sequence_name, detection in detection_dict.items():
            ...
            torch.save(detection, output_detection_path)
        print("Done!")
